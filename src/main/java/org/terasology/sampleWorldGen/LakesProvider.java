...
public class LakesProvider implements FacetProviderPlugin {
    ...
    public void setSeed(long seed) {
        lakeNoise = new SubSampledNoise(new BrownianNoise(new PerlinNoise(seed + 5), 4), new Vector2f(0.001f, 0.001f), 1);
    }
    ...
    public void process(GeneratingRegion region) {
        ...
        float lakeDepth = 40;
        // loop through every position on the 2d array
        Rect2i processRegion = facet.getWorldRegion();
        for (BaseVector2i position : processRegion.contents()) {
            float additiveLakeDepth = lakeNoise.noise(position.x(), position.y()) * lakeDepth;
            // Don't bother adding lake height,  that will allow unaffected regions
